...
// {"web_feature":<web_feature_string>}.
func (t *AbstractTestWebFeature) UnmarshalJSON(b []byte) error {
	var data map[string]*json.RawMessage
	if err := json.Unmarshal(b, &data); err != nil {
		return err
	}
	webFeatureMsg, ok := data["web_feature"]
	if !ok {
		return errors.New(`Missing web feature pattern property: "web_feature"`)
	}
	var webFeature string
	if err := json.Unmarshal(*webFeatureMsg, &webFeature); err != nil {
		return errors.New(`Property "web_feature" is not a string`)
	}

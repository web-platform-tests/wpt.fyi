...
def cleanup_stuck_runs(days_threshold=14):
    ...
    for run in stuck_runs:
        ...
        if updated and updated < cutoff:
            ...
            _log.info("Marking run %s as INVALID (last updated %s)", run_id, updated)
            try:
                # Use the update_status method from Processor which handles the API call
                p.update_status(run_id, 'INVALID', error='Run timed out after {} days'.format(days_threshold))
                count += 1

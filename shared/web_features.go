...
func (p WebFeaturesManifestJSONParser) Parse(ctx context.Context, r io.ReadCloser) (WebFeaturesData, error) {
	...
	return nil, fmt.Errorf("unknown web features manifest version: %d", file.Version)
}
...
func (d webFeaturesManifestV1Data) prepareTestWebFeatureFilter() map[string][]string {
	...
	for webFeature, tests := range d {
		for _, test := range tests {
			if currentTests, ok := testToWebFeaturesMap[test]; !ok {
				testToWebFeaturesMap[test] = []string{webFeature}
			} else {
				testToWebFeaturesMap[test] = append(currentTests, test)
			}
